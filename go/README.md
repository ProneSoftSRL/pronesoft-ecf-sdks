...
Import the package in your project and run go mod tidy:
...
4. Reuse session.Client() while that tenant stays active.
...
response, httpResp, err := client.ECFSubmissionAPI.
...
```go
if err := session.SetCurrentTenant("101234567"); err != nil {
	log.Fatal(err)
}
```
