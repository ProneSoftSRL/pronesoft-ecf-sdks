...
module PronesoftEcf
  class SentDocumentDetail < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PronesoftEcf::SentDocumentDetail` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PronesoftEcf::SentDocumentDetail`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, encf, status, status_label, track_id, document_type, issuer_rnc, environment, received_at, created_at, business, legal_status, document_stamp_url, security_code, contingency_mode, government_response].hash
    end
