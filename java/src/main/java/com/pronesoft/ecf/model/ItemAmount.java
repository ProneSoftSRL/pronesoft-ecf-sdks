...
public class ItemAmount extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(ItemAmount.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ItemAmount.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ItemAmount' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<ItemAmount>() {
                @Override
                public void write(JsonWriter out, ItemAmount value) throws IOException {
                ...
                @Override
                public ItemAmount read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        ItemAmount ret = new ItemAmount();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for ItemAmount: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to ItemAmount
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for ItemAmount with oneOf schemas: BigDecimal, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of ItemAmount given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of ItemAmount
     * @throws IOException if the JSON string is invalid with respect to ItemAmount
     */
    public static ItemAmount fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, ItemAmount.class);
    }

    /**
     * Convert an instance of ItemAmount to an JSON string
     *
     * @return JSON string
     */
