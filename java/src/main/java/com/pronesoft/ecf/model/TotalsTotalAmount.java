...
public class TotalsTotalAmount extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(TotalsTotalAmount.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!TotalsTotalAmount.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'TotalsTotalAmount' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<TotalsTotalAmount>() {
                @Override
                public void write(JsonWriter out, TotalsTotalAmount value) throws IOException {
                ...
                @Override
                public TotalsTotalAmount read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        TotalsTotalAmount ret = new TotalsTotalAmount();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for TotalsTotalAmount: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public TotalsTotalAmount() {
        super("oneOf", Boolean.FALSE);
    }

    public TotalsTotalAmount(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return TotalsTotalAmount.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to TotalsTotalAmount
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for TotalsTotalAmount with oneOf schemas: BigDecimal, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of TotalsTotalAmount given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of TotalsTotalAmount
     * @throws IOException if the JSON string is invalid with respect to TotalsTotalAmount
     */
    public static TotalsTotalAmount fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, TotalsTotalAmount.class);
    }

    /**
     * Convert an instance of TotalsTotalAmount to an JSON string
     *
     * @return JSON string
     */
