...
public class ItemUnitPrice extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(ItemUnitPrice.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ItemUnitPrice.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ItemUnitPrice' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<ItemUnitPrice>() {
                @Override
                public void write(JsonWriter out, ItemUnitPrice value) throws IOException {
                ...
                @Override
                public ItemUnitPrice read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        ItemUnitPrice ret = new ItemUnitPrice();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for ItemUnitPrice: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to ItemUnitPrice
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for ItemUnitPrice with oneOf schemas: BigDecimal, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of ItemUnitPrice given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of ItemUnitPrice
     * @throws IOException if the JSON string is invalid with respect to ItemUnitPrice
     */
    public static ItemUnitPrice fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, ItemUnitPrice.class);
    }

    /**
     * Convert an instance of ItemUnitPrice to an JSON string
     *
     * @return JSON string
     */
